...
bool_t dpk4_read_dir(restable_t * rt)
{
  ...
  if (readf(rt->file, "c4l4l4l4", ident, &filesize, &dirsize, &number) != OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (readf(rt->file, "l4l4l4l4sz",
              ...
              &(rt->entries[i].offset), &(rt->entries[i].name)) != OK)
    {
      fprintf(stderr, "dpk4_read_dir: Can't read entry #%zu.\n", i);
      return FALSE;
    }
    ...
    {
      fprintf(stderr,
              "dpk4_read_dir: Can't skip padding bytes of entry #%zu.\n", i);
      return FALSE;
    }
  }
  return TRUE;
...
bool_t dpk4_extract_resource(restable_t * rt, size_t i)
{
  char *filename;
  ...
  if (c_extractfile(filename, rt->file, rt->entries[i].size,
                 rt->entries[i].compressed <
                 rt->entries[i].size ? rt->entries[i].compressed : 0,
                 &comdec_zlib) == FALSE)
  {
    fprintf(stderr, "dpk4_extract_resource: Can't extract entry #%zu.\n", i);
    s_free(&filename);
    return FALSE;
  }
  s_free(&filename);
...
bool_t dpk4_prepare_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
    offset +=
      align(DPK4_ENTRY_SIZE + strlen(rt->entries[i].name) + 1, DPK4_ENTRY_PAGE);
  fseek(rt->file, offset, SEEK_SET);
  return TRUE;
}

bool_t dpk4_add_resource(restable_t * rt, size_t i)
{
  char *filename;
  ...
  {
    fprintf(stderr, "dpk4_add_resource: Can't add entry #%zu.\n", i);
    s_free(&filename);
    return FALSE;
  }
...
bool_t dpk4_write_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
    size +=
      align(DPK4_ENTRY_SIZE + strlen(rt->entries[i].name) + 1, DPK4_ENTRY_PAGE);
  if (writef(rt->file, "c4l4l4l4", DPK4_IDENT, offset, size, rt->number) != OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    ...
    if (writef(rt->file, "l4l4l4l4szzn",
               ...
               rt->entries[i].compressed,
               rt->entries[i].offset,
               rt->entries[i].name, aligned - size) != OK)
    {
      fprintf(stderr, "dpk4_write_dir: Can't write entry #%zu.\n", i);
      return FALSE;
    }
  }
  fseek(rt->file, offset, SEEK_SET);
