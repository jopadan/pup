...
bool_t pack2_read_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
  {
    ...
    {
      fprintf(stderr, "pack2_read_dir: Can't read entry #%zu.\n", i);
      return FALSE;
    }
...
bool_t pack2_extract_resource(restable_t * rt, size_t i)
{
  char *filename;
  ...
  {
    fprintf(stderr, "pack2_unpack_files: Can't extract entry #%zu.\n", i);
    s_free(&filename);
    return FALSE;
  }
  s_free(&filename);
...
bool_t pack2_add_resource(restable_t * rt, size_t i)
{
  char *filename;
...
bool_t pack2_write_dir(restable_t * rt)
{
  ...
  if (writef
      (rt->file, "c4l4l4", PACK2_IDENT, offset,
       rt->number * PACK2_ENTRY_SIZE) != OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (writef(rt->file, "s56l4l4l4l4",
               ...
               rt->entries[i].offset,
               rt->entries[i].size,
               rt->entries[i].compressed, rt->entries[i].compression) != OK)
    {
      fprintf(stderr, "pack2_write_dir: Can't write entry #%zu.\n", i);
      return FALSE;
    }
  }
  return TRUE;
