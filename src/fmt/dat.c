...
bool_t dat_extract_resource(restable_t * rt, size_t i)
{
  char *filename;
  ...
  if (c_extractfile(filename, rt->file, rt->entries[i].size,
                 ...
                 0x40 ? rt->entries[i].compressed : 0, &comdec_dat) == FALSE)
  {
    fprintf(stderr, "dat_unpack_files: Can't extract entry #%zu.\n", i);
    s_free(&filename);
    return FALSE;
  }
  s_free(&filename);
...
bool_t dat_add_resource(restable_t * rt, size_t i)
{
  char *filename;
  ...
  if (c_addfile(rt->file, filename, &(rt->entries[i].size),
             &(rt->entries[i].compressed), &comdec_dat, 9) == FALSE)
  {
    fprintf(stderr, "dat_add_resource: Can't add entry #%zu.\n", i);
    s_free(&filename);
    return FALSE;
  }
...
bool_t dat_set_numpaths(dat_t * dat, size_t np)
{
  size_t i;
  size_t mp;
  dat_path_t *p;
...
ssize_t dat_add_path(dat_t * dat, const char *path)
{
  size_t i;
...
bool_t dat_set_numfiles(dat_t * dat, size_t nf)
{
  size_t i;
  size_t mf;
  dat_file_t *f;
...
bool_t dat_to_rt(restable_t * rt, dat_t * dat)
{
  ...
  for(i = 0; i < dat->numfiles; i++)
  {
    s_strcpy(&(rt->entries[i].name), dat->paths[dat->files[i].path].name);
...
void dat_print(dat_t * dat)
{
  ...
  for(i = 0; i < dat->numfiles; i++)
    fprintf(stderr, "\t%zu\t%zu\t%s\t0x%02zX\t0x%08zX\t%zu\t%zu\n",
            i,
            dat->files[i].path,
            dat->files[i].name,
            dat->files[i].compression,
            dat->files[i].offset, dat->files[i].size, dat->files[i].compressed);
}
...
bool_t dat_read_dir(restable_t * rt)
{
  ...
  if (readf(rt->file, "b4c4b4c4", &number, NULL, &zero, NULL) != OK)
  {
  ...
  for(i = 0; i < dat.numpaths; i++)
  {
    ...
    {
      fprintf(stderr, "dat_read_dir: Can't read path name #%zu.\n", i);
      dat_free(&dat);
      return FALSE;
    }
  }
  for(i = 0; i < dat.numpaths; i++)
  {
    if (readf(rt->file, "b4c12", &(dat.paths[i].numfiles), NULL) != OK)
    {
...
bool_t dat_from_rt(restable_t * rt, dat_t * dat)
{
  ...
  for(i = 0; i < rt->number; i++)
  {
    ...
    /* Добавление файла */
    s_nameext(&(dat->files[i].name), rt->entries[i].name, DAT_PATH_DELIM);
    dat->files[i].path = j;
...
bool_t dat_write_dir(restable_t * rt)
{
  ...
  if (writef(rt->file, "b4b4b4b4", dat.numpaths, dat.numpaths, 0, 0) != OK)
  {
  ...
  for(i = 0; i < dat.numpaths; i++)
  {
    ...
    {
      fprintf(stderr, "dat_write_dir: Can't write path name #%zu.\n", i);
      dat_free(&dat);
      return FALSE;
    }
  }
  ...
  for(i = 0; i < dat.numpaths; i++)
  {
    if (writef(rt->file, "b4b4b4b4",
               dat.paths[i].numfiles, dat.paths[i].numfiles, 0x10, 0) != OK)
    {
      fprintf(stderr, "dat_write_dir: Can't write entry path header #%zu.\n",
              i);
      dat_free(&dat);
      return FALSE;
    }
    for(j = 0; j < dat.paths[i].numfiles; j++)
    {
      if (writef(rt->file, "spb4b4b4b4",
                 ...
                 dat.files[number + j].compressed) != OK)
      {
        fprintf(stderr, "dat_write_dir: Can't write entry #%zu of path #%zu.\n",
                j, i);
        dat_free(&dat);
        return FALSE;
      }
