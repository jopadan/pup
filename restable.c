...
bool_t rt_init(restable_t * rt, const char *filename, const char *basepath,
               const char *meta, const char *mode)
{
  rt_clear(rt);
...
bool_t rt_realloc(restable_t * rt, size_t maxnumber)
{
  resentry_t *e;
  size_t i;
...
bool_t rt_fill_entries0(restable_t * rt, const char *path, bool_t subdirs)
{
  ...
  while ((de = readdir(d)) != NULL)
  {
    ...
    {
      fprintf(stderr, "rt_fill_entries: Can't detect type of file %s.\n",
              filename);
      continue;
    }
        ...
        (strcmp(de->d_name, ".") != 0) && (strcmp(de->d_name, "..") != 0))
    {
      if (subdirs == FALSE)
      {
        fprintf(stderr, "rt_fill_entries: Can't add subdirectory %s.\n",
                filename);
        s_free(&filename);
        closedir(d);
        return FALSE;
...
void rt_print_dir(restable_t * rt, FILE * file)
{
  ...
  for(i = 0; i < rt->number; i++)
  {
    ...
    fprintf(file, "\n"
            ...
            rt->entries[i].id,
            rt->entries[i].copyof,
            rt->entries[i].name, rt->entries[i].filename);
  }
  s_free(&s_t);
}
  ...
  for(i = 0; i < rt->number - 1; i++)
  {
    for(j = i + 1; j < rt->number; j++)
    {
      ...
      if (((rt->entries[i].offset > rt->entries[j].offset)
          ...
          ((rt->entries[i].offset + rt->entries[i].compressed >
            rt->entries[j].offset) &&
           (rt->entries[i].offset + rt->entries[i].compressed <
            rt->entries[j].offset + rt->entries[j].compressed)))
      {
    ...
    fprintf(stderr,
            "Checking entries %zu/%zu. Found %zu referenced to same block, %zu with same content, %zu entries with same name.\r",
            i, rt->number, hardlinks, copies, renamed);
  }
  fprintf(stderr,
          "Checking entries finished. Found %zu referenced to same block, %zu with same content, %zu entries with same name.\n",
...
bool_t rt_search_equal_files(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number - 1; i++)
  {
    ...
    fprintf(stderr,
            "Searching of files with same content %zu/%zu. Found %zu files.\r",
            i, rt->number, copies);
  }
  fprintf(stderr,
