...
size_t max[] = { 0x000000FF, 0x0000FFFF, 0x00FFFFFF, 0xFFFFFFFF };

binrw_error_t writel(FILE * file, size_t num, size_t len)
{
  size_t i;

  if ((file == NULL) || (len < 1) || (len > sizeof(num)) ||
      (num > max[len - 1]))
    return ARG_ERROR;
...
binrw_error_t unpackl(buf_t * buf, size_t * pnum, size_t len)
{
  ...
  if ((buf == NULL) || (buf->p == NULL) || (pnum == NULL) || (len < 1) ||
      (len > sizeof(*pnum)))
    return ARG_ERROR;
...
binrw_error_t packl(buf_t * buf, size_t num, size_t len)
{
  ...
  if ((buf == NULL) || (buf->p == NULL) || (len < 1) || (len > sizeof(num)) ||
      (num > max[len - 1]))
    return ARG_ERROR;
  if (buf->offset + len > buf->size)
    return WRITE_ERROR;
...
time_t unix2dos_timestamp(const time_t timestamp)
{
  ...
  ts = (1 << DOS_MONTHS_SHIFT) | (1 << DOS_DAYS_SHIFT);
